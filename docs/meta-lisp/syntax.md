...
- [Algebraic data types](#algebraic-data-types)
  ...
  - [(define-struct-type)](#define-struct-type)
...
| `(define-struct-type)`      | Single constructor, all names explicit |
...
(define-struct-type (<type-name> <type-parameter> ...)
  (<constructor-name> (<field-name> <type>) ...)
  <predicate-name>
  (<field-name> <accessor-name> <modifier-name>)
  ...)
...
The `(define-struct-type)` syntax comes from Scheme,
originating from the Scheme 48 dialect,
where it was called `define-record-type`.

We have extended this syntax with type annotations for `<field-name>`,
i.e. `(<field-name> <type>)`.

`(define-algebraic-type)` is modeled after `(define-struct-type)`,
